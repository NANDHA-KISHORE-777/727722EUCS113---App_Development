...
const Dashboard = () => {
  const [lowStockItems, setLowStockItems] = useState(7);
  const [allItems, setAllItems] = useState(93);
  const [packed, setPacked] = useState(5);
  const [shipped, setShipped] = useState(10);
  const [delivered, setDelivered] = useState(25);

  const data = [
    { name: 'Total No of Products', value: allItems },
    { name: 'Low Stock Items', value: lowStockItems },
  ];
